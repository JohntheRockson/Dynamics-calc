...
`EnvironmentalTorques` via the same `--gravity-gradient` / `--residual-dipole`
path (ZOH at each sample).  `--mrp-plot` writes a post-process MRP chart from
logged \(q\) (default on for hold / eigenaxis).  They can share the wheel box:
