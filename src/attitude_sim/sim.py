...
def make_scenario_config(
    ...
    *,
    ...
    dipole.  ``--no-env`` turns those two off.  Aero / SRP stay independent
    opt-in flags.  Orbit / dipole / panel knobs are stored even when off.
    """
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--no-env",
        action="store_true",
        help="disable GG + residual dipole (overrides hold / --env / --gravity-gradient / --residual-dipole)",
    )
    p.add_argument(
        "--coarse-init",
        action="store_true",
