...
def test_mekf_nees_honest_prior_matches_chi2():
    ...
    values = [nees(rng.multivariate_normal(np.zeros(6), P0), P0) for _ in range(n)]
...
def test_mekf_nees_closed_loop_slew_is_finite_and_bounded():
    ...
    assert values.min() > 0.0
    mean = float(values.mean())
    # Matched open-loop 99% mean bounds are ~[4.54, 7.69].  Closed-loop is
    # allowed to sit well outside that; only explode / NaN is a failure.
    lo, hi = chi2_mean_nees_bounds(MEKF_NEES_DOF, n, alpha=0.01)
    assert mean < 80.0, (
