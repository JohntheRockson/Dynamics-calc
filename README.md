...
python -m attitude_sim --controller pid --estimator truth \
    --gravity-gradient --residual-dipole 0.2,-0.1,0.5 \
    --orbit-incl-deg 51.6 --no-gif
python -m attitude_sim --help
```

`--angle-deg` sets the commanded principal rotation for `--scenario slew` and is ignored for `detumble`. `--tau-dist` is a constant body-frame disturbance added to the plant only. Gravity-gradient and residual-dipole models default **off**; when enabled they are ZOH at the sample and added with `--tau-dist`.
...
| `--gravity-gradient` / `--residual-dipole MX,MY,MZ` | Optional environmental torques from `attitude_sim.disturbances` (default **off**). Both are ZOH at the sample and added to `--tau-dist`. `--orbit-radius` (m), `--orbit-incl-deg`, `--orbit-raan-deg`, `--orbit-u0-deg`, `--mag-field-model {tilted,orbit_normal}`. |
...
- CLI smoke: no-plot / **no-GIF** SimLab slew, detumble, LQR+Mahony, PID hold with `--tau-dist`, actuator saturation/lag, gravity-gradient + residual-dipole, sensor-noise knobs, plus a tiny Monte Carlo entry (`python -m attitude_sim.monte_carlo --n 5`, short `t_final`, `--diverge-deg 180` so a 0.2 s run is scored for numerical health rather than settling; detumble is not in the MC harness). Attitude GIF rendering is not a CI gate.
...
**Environmental torques** (`attitude_sim.disturbances`; optional SimLab plant inputs, default **off**). Orbit frame: \(\hat r\) zenith (Earth→s/c), \(\hat h\) orbit normal \(r\times v\), nadir \(-\hat r\). Body vectors are \(v_b=R(q)^{\top}v_I\). Gravity-gradient (circular or given orbit state)

\[
\tau_{\mathrm{gg}} = 3\frac{\mu}{r^{3}}\,(\hat r_b \times J\hat r_b)
\]

vanishes when a principal axis of \(J\) lies along nadir/zenith. Residual dipole \(\tau_m=m_b\times B_b\) with \(B_b=R(q)^{\top}B_I\) and \(B_I\) either a tilted Earth dipole or \(B_I=(\mu_m/r^{3})\hat h\) (orbit-normal / equatorial-dipole option). Parallel \(m\) and \(B\) give zero torque. Call `τ_body(q, ω, t)` with a bound `CircularOrbit`, or pass `orbit=OrbitState(...)`. SimLab injects them in `run_slew` as a sample ZOH alongside `--tau-dist` when `--gravity-gradient` and/or `--residual-dipole` are set. Logged \(\tau\) remains the applied wheel torque; `SimLog.tau_env` is the environmental sample.
...
sensors  →  estimator (MEKF / Mahony / truth)
            ...
            actuator (per-axis clip, optional lag)  →  τ
                 ↓
            + τ_d (constant) + τ_env (optional GG / residual dipole, default off)
                 ↓
...
| `attitude_sim.disturbances` | Gravity-gradient and residual-dipole `τ_body(q, ω, t or orbit)`; SimLab `--gravity-gradient` / `--residual-dipole` (default off) |
...
| `pid` or `lqr` | `truth` | Body-torque hold: `--angle-deg 0 --tau-dist …` and/or `--gravity-gradient` / `--residual-dipole` |
